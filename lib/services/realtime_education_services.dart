...
class RealtimeEducationService {
  ...
  Future<void> initializeEducationData() async {
    try {
      ...
      final snapshot = await _databaseRef.child('education').get();
      if (snapshot.exists && snapshot.value != null) {
        print('Data edukasi sudah ada di Realtime Database');
        return;
      }
...
- Cocok untuk sistem vertikal maupun horizontal skala rumah tangga''',
      ...
      for (var plant in plants) {
        await _databaseRef.child('education').child(plant['id']).set(plant);
      }

      print('Data edukasi berhasil diinisialisasi di Realtime Database');
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getEducationData() async {
    try {
      ...
      // Konversi data dari Realtime Database ke List<Map>
      final data = snapshot.value as Map<dynamic, dynamic>;
      return data.entries.map((entry) {
        final plantData = entry.value as Map<dynamic, dynamic>;
        return {
          'id': entry.key,
          ...plantData.map((key, value) => MapEntry(key.toString(), value)),
        };
      }).toList();
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> getEducationById(String id) async {
    try {
      final snapshot = await _databaseRef.child('education').child(id).get();
      if (snapshot.exists && snapshot.value != null) {
        final data = snapshot.value as Map<dynamic, dynamic>;
        return {
          'id': id,
  ...
  Future<List<Map<String, dynamic>>> searchEducation(String query) async {
    try {
      final allData = await getEducationData();
      return allData.where((plant) {
